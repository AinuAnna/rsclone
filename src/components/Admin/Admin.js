...
<tbody>
    ${users
      .map(
        (item) =>
          `<tr data-id="${item.id}"><td>${item.fullName}</td><td>${item.id}</td><td>${item.mail}</td><td>${item.password}</td><td>${item.type}</td><td>${item.description}</td><td class="table__action">
        <a href="#" class = "edit"
          ...
          <button class = "delete" data-bs-userid="${item.id}" data-bs-username="${item.fullName}" data-bs-target="#myModal" data-bs-toggle="modal"
