...
export default class Teacher extends UI {
  ...
  renderGroup() {
    ...
    const tableTitles = ['Цикловая комиссия', 'Курс', 'Группа', 'Описание', 'Участники', 'Действия'];
  ...
  addNextForm() {
    ...
    UI.renderElement(tr3, 'td', 'Группа', ['class', 'number']);
    ...
    ButtonAddGruop.addEventListener('click', (e) => {
      ...
      this.renderGroup();
      this.renderStudentsGroup();
    });
  }
  renderStudentsGroup() {
    const wrapper = UI.renderElement(this.rootNode, 'div', null, ['class', 'group__wrapper']);
    const tableTitles = ['ФИО', 'Эл.Почта', 'Описание', 'Результаты тестов', 'Действия'];
    UI.renderElement(wrapper, 'h2', 'Участники группы', ['class', 'group__title']);
    const container = UI.renderElement(wrapper, 'div', null, ['class', 'table-responsive-md']);
    const table = UI.renderElement(container, 'table', null, ['class', 'table']);
    const thead = UI.renderElement(table, 'thead');
    const trh = UI.renderElement(thead, 'tr');
    tableTitles.forEach((title) => UI.renderElement(trh, 'th', title));
    const tbody = UI.renderElement(table, 'tbody');
    this.usersArray.forEach(({ id, fullName, mail, description }) => {
      const tr = UI.renderElement(tbody, 'tr', null, ['data-id', id]);
      UI.renderElement(tr, 'td', fullName, ['class', 'email']);
      UI.renderElement(tr, 'td', mail, ['class', 'email']);
      UI.renderElement(tr, 'td', description, ['class', 'description']);
      const participantsTR = UI.renderElement(tr, 'td', null, ['class', 'participants']);
      UI.renderElement(participantsTR, 'a', 'просмотреть', ['class', 'participants']);
      UI.renderElement(
        tr,
        'a',
        `<svg width="15" height="15" viewBox="0 0 22 23" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    UI.renderElement(table, 'tbody');
    const ButtonAddStudent = UI.renderElement(
      wrapper,
      'a',
      'Добавить студента',
      ['href', '#'],
  ...
  addStudent() {
    const wrapper = UI.renderElement(this.rootNode, 'div', null, ['class', 'group__wrapper']);
    const tableTitles = ['Параметры студента'];
    UI.renderElement(wrapper, 'h2', 'Добавление студента', ['class', 'group__title ']);
    const container = UI.renderElement(wrapper, 'div', null, ['class', 'table-responsive-md']);
    const table = UI.renderElement(container, 'table', null, ['class', 'table']);
    const thead = UI.renderElement(table, 'thead');
    const trh = UI.renderElement(thead, 'tr');
    ...
    const tbody = UI.renderElement(table, 'tbody');
    const tr1 = UI.renderElement(tbody, 'tr', null);
    UI.renderElement(tr1, 'td', 'ФИО', ['class', 'department']);
    const tr2 = UI.renderElement(tbody, 'tr', null);
    UI.renderElement(tr2, 'td', 'Эл.Почта', ['class', 'course']);
    UI.renderElement(trh, 'th', null);
    UI.renderElement(trh, 'tr', null);
    const selectStudent = UI.renderElement(
      tr1,
      'select',
      null,
      ['class', 'form-control title-test-input'],
    ...
    this.usersArray.forEach(({ id, fullName }) => {
      const tr = UI.renderElement(tbody, 'tr', null, ['data-id', id]);
      UI.renderElement(selectStudent, 'option', fullName, ['value', `${fullName}`]);
    });
    UI.renderElement(trh, 'tr', null);
    const selectStudentMail = UI.renderElement(
      tr2,
      'select',
      null,
      ['class', 'form-control title-test-input'],
    ...
    this.usersArray.forEach(({ id, mail }) => {
      const tr = UI.renderElement(tbody, 'tr', null, ['data-id', id]);
      UI.renderElement(selectStudentMail, 'option', mail, ['value', `${mail}`]);
    });
    UI.renderElement(tbody, 'tr', null);
    const ButtonAddGruop = UI.renderElement(wrapper, 'a', 'Сохранить', ['href', '#'], ['class', 'btn btn-primary']);
    ButtonAddGruop.addEventListener('click', (e) => {
