...
export default class LectureAdmin extends UI {
  ...
  renderM() {
    ...
    this.lecturesArray.forEach(({ id, title }) => {
      ...
      this.lecturesArray.forEach(({ subtitle }) => {
    ...
    const titleAdd2 = UI.renderElement(titleAddTests, 'div', 'Введите тему:', ['class', 'lecture-admin__items-add']);
    ...
    const optionalInput = UI.renderElement(divInputText, 'input', null, ['type', 'file'], ['value', 'Выберите файл']);
    UI.renderElement(
      this.rootNode,
      'button',
      'Добавить лекцию',
