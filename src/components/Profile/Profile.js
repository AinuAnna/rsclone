...
export default class Profile extends UI {
  ...
  renderM() {
    ...
    const cardM3 = UI.renderElement(right, 'div', null, ['class', 'card-body text-center']);
    const cardHeader = UI.renderElement(cardM3, 'div', null, ['class', 'card-header']);
    UI.renderElement(cardHeader, 'h5', 'Фото профиля', ['class', 'student-profile__photo-title card-title mb-0']);
    const cardBody = UI.renderElement(cardM3, 'div', null, ['class', 'card-body text-center']);
    UI.renderElement(
      cardBody,
      'img',
      null,
      ['class', 'student-profile__photo img-fluid rounded-circle mb-2'],
      [
        'src',
    ...
    const divInputs = UI.renderElement(cardBody, 'div', null, ['data-id', this.studentInfo.id], ['class', 'divInputs']);
